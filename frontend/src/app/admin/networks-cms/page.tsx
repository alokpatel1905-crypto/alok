...
export default function ManageNetworksPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      setMessage('');
      await apiFetch('/networks-page/update', {
        method: 'POST',
        body: JSON.stringify(formData)
      });
      setMessage('✅ Network protocols synchronized successfully!');
  ...
  const NetworkBlock = ({ prefix, title, icon: Icon }: { prefix: string, title: string, icon: any }) => (
    <div className="bg-white/60 backdrop-blur-xl border border-primary/10 rounded-[3rem] p-12 space-y-10 shadow-premium animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <Field label="Network Identity" field={`${prefix}_title`} />
      ...
      <Field label="Institutional Logo URL" field={`${prefix}_image`} placeholder="https://" />
    </div>
  );

  const sections = [
    { id: 'intro', label: 'Genesis Intro', icon: Layout },
    { id: 'school', label: 'Green Schools', icon: Network },
    { id: 'university', label: 'Green University', icon: Globe },
    { id: 'teacher', label: 'Green Teacher', icon: Users },
    { id: 'graduates', label: 'Green Graduates', icon: Target },
    { id: 'innovator', label: 'Innovator Hub', icon: Zap },
    { id: 'why', label: 'Manifesto', icon: Star },
    { id: 'impact', label: 'Network Impact', icon: Share2 },
    { id: 'process', label: 'Participation Flow', icon: ArrowRight },
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
      ...
      <p className="text-sm font-black uppercase tracking-widest text-foreground/40 animate-pulse">Syncing Network Matrix...</p>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white/40 backdrop-blur-xl border border-primary/5 rounded-[3rem] p-10 shadow-premium">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-4xl font-display font-black text-foreground tracking-tighter uppercase">Networks CMS</h1>
          </div>
          <p className="text-foreground/40 font-medium italic pl-1">Configuring the global network hubs, benefits, and institutional collaboration protocols.</p>
        </div>
        <Link 
          href="/networks" 
          target="_blank" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        ...
        <form onSubmit={handleSave} className="lg:col-span-3 space-y-10">
          ...
          {/* NETWORKS */}
          {activeSection === 'school' && <NetworkBlock prefix="school" title="Green Schools Network" icon={Network} />}
          {activeSection === 'university' && <NetworkBlock prefix="university" title="Green University Network" icon={Globe} />}
          {activeSection === 'teacher' && <NetworkBlock prefix="teacher" title="Green Teacher Network" icon={Users} />}
          {activeSection === 'graduates' && <NetworkBlock prefix="graduates" title="Green Graduates Network" icon={Target} />}
          {activeSection === 'innovator' && <NetworkBlock prefix="innovator" title="Green Innovator Network" icon={Zap} />}
          ...
          {activeSection === 'why' && (
            <div className="bg-white/60 backdrop-blur-xl border border-primary/10 rounded-[3rem] p-12 space-y-10 shadow-premium animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <Field label="Benefits Narrative" field="why_description" type="textarea" placeholder="Use hyphens for bullets..." />
            </div>
          )}
          ...
          {activeSection === 'impact' && (
            <div className="bg-white/60 backdrop-blur-xl border border-primary/10 rounded-[3rem] p-12 space-y-10 shadow-premium animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-4 border-b border-primary/5 pb-8">
                ...
                <h2 className="text-2xl font-black text-foreground tracking-tighter uppercase">Network Impact</h2>
              </div>
              <Field label="Impact Header" field="impact_title" />
              <Field label="Impact Narrative" field="impact_description" type="textarea" />
            </div>
          )}
          ...
          {activeSection === 'process' && (
            <div className="bg-white/60 backdrop-blur-xl border border-primary/10 rounded-[3rem] p-12 space-y-10 shadow-premium animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-4 border-b border-primary/5 pb-8">
                ...
                <h2 className="text-2xl font-black text-foreground tracking-tighter uppercase">Participation Flow</h2>
              </div>
