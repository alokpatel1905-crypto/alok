...
export default function ManageRankingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('✅ Ranking protocols synchronized successfully!');
  ...
  const RankingBlock = ({ prefix, title, icon: Icon }: { prefix: string, title: string, icon: any }) => (
  ...
  const sections = [
    ...
    { id: 'regional', label: 'Regional Ranking', icon: Globe },
    { id: 'why', label: 'Participation Manifesto', icon: Star },
    { id: 'methodology', label: 'Global Methodology', icon: TrendingUp },
    { id: 'process', label: 'Ranking Flow', icon: ListChecks },
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
      ...
      <p className="text-sm font-black uppercase tracking-widest text-foreground/40 animate-pulse">Syncing Ranking Matrix...</p>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white/40 backdrop-blur-xl border border-primary/5 rounded-[3rem] p-10 shadow-premium">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-4xl font-display font-black text-foreground tracking-tighter uppercase">Rankings CMS</h1>
          </div>
          <p className="text-foreground/40 font-medium italic pl-1">Monitoring the global benchmarks of sustainable institutional performance.</p>
        </div>
        <Link 
          href="/rankings" 
          target="_blank" 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        ...
        <form onSubmit={handleSave} className="lg:col-span-3 space-y-10">
          ...
          {/* RANKING PROGRAMS */}
          {activeSection === 'school' && <RankingBlock prefix="school" title="Green School Ranking" icon={School} />}
          {activeSection === 'university' && <RankingBlock prefix="university" title="Green University Ranking" icon={Trophy} />}
          {activeSection === 'regional' && <RankingBlock prefix="regional" title="National & Regional Ranking" icon={Globe} />}
          ...
          {activeSection === 'methodology' && (
            <div className="bg-white/60 backdrop-blur-xl border border-primary/10 rounded-[3rem] p-12 space-y-10 shadow-premium animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-4 border-b border-primary/5 pb-8">
                ...
                <h2 className="text-2xl font-black text-foreground tracking-tighter uppercase">Global Methodology</h2>
              </div>
              <Field label="Methodology Header" field="methodology_title" />
              <Field label="Framework Parameters" field="methodology_description" type="textarea" placeholder="Use hyphens for bullets..." />
            </div>
          )}
          ...
          {activeSection === 'process' && (
            <div className="bg-white/60 backdrop-blur-xl border border-primary/10 rounded-[3rem] p-12 space-y-10 shadow-premium animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="flex items-center gap-4 border-b border-primary/5 pb-8">
                ...
                <h2 className="text-2xl font-black text-foreground tracking-tighter uppercase">Ranking Flow</h2>
              </div>
