...
export default function ManageRankingsPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const data = await apiFetch('/rankings-page');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      setMessage('');
      await apiFetch('/rankings-page/update', {
        method: 'POST',
        body: JSON.stringify(formData)
      });
      setMessage('✅ Rankings page configuration saved successfully!');
  ...
  const RankingBlock = ({ prefix, title }: { prefix: string, title: string }) => (
    <div className="bg-white p-8 rounded-lg shadow-sm border border-slate-200 mb-6 border-l-4 border-l-emerald-600">
      <h2 className="text-lg font-bold text-slate-800 mb-6 border-b pb-3 uppercase tracking-wide">{title}</h2>
      <div className="grid md:grid-cols-2 gap-6">
        <Field label="Ranking Title" field={`${prefix}_title`} />
        <Field label="Ranking Subtitle" field={`${prefix}_subtitle`} />
      </div>
      <Field label="Ranking Description" field={`${prefix}_description`} type="textarea" />
      ...
      <div className="mt-6">
        <Field label="Ranking Image / Logo URL" field={`${prefix}_image`} placeholder="https://" />
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Manage Rankings Page</h1>
          <p className="text-sm text-slate-500 mt-1">Configure the ranking systems, methodology, and processes.</p>
        </div>
        <Link href="/rankings" target="_blank" className="text-sm text-emerald-600 font-semibold hover:underline flex items-center gap-1">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {/* SECTION 2 - 4: RANKING PROGRAMS */}
        <RankingBlock prefix="school" title="SECTION 2: Global Green School Ranking" />
        <RankingBlock prefix="university" title="SECTION 3: Global Green University Ranking" />
        <RankingBlock prefix="regional" title="SECTION 4: National & Regional Rankings" />

        {/* SECTION 5: WHY PARTICIPATE */}
        <div className="bg-white p-8 rounded-lg shadow-sm border border-slate-200">
          <h2 className="text-lg font-bold text-slate-800 mb-6 border-b pb-3">SECTION 5: WHY PARTICIPATE</h2>
        ...
        <div className="bg-white p-8 rounded-lg shadow-sm border border-slate-200">
          <h2 className="text-lg font-bold text-slate-800 mb-6 border-b pb-3">SECTION 6: METHODOLOGY OVERVIEW</h2>
          <Field label="Methodology Title" field="methodology_title" />
          <Field label="Methodology Description (Add bullets via hyphens)" field="methodology_description" type="textarea" />
        </div>

        {/* SECTION 7: PROCESS FLOW */}
        <div className="bg-white p-8 rounded-lg shadow-sm border border-slate-200 bg-slate-50 border-dashed border-2">
          <h2 className="text-lg font-bold text-slate-800 mb-6 border-b pb-3">SECTION 7: RANKING PROCESS FLOW</h2>
          <Field label="Process Main Title" field="process_title" />
        ...
        <div className="flex items-center gap-4 pt-4 sticky bottom-6 bg-slate-100/80 backdrop-blur p-4 rounded-2xl border border-slate-200/50 shadow-2xl z-50">
          ...
          >
            {saving ? 'Saving...' : 'Save Ranking Configurations'}
          </button>
